...
function check_oracle_container() {
    container_name=$1
    ORACLE_USER=$2
    ORACLE_PWD=$3
    ...
    while true; do
        ...
        if [ "$status" == "running" ]; then
            ...
            if [ "$DC_FILE" == "docker-compose.yaml" ]; then
                # Oracle Database takes a while to start up, check if it's ready to accept connections
                # ready=$(docker exec $container_name /bin/bash -c "echo 'SELECT 1 FROM DUAL;' | sqlplus -s / ")
                ready=$(docker compose -f ${DC_FILE} logs ${container_name})

                if [[ "$ready" == *"DATABASE IS READY TO USE!"* ]]; then
...
function check_debezium_connect_container() {
    ...
    while true; do
        ...
        if [ "$status" == "running" ]; then
            ...
            ready=$(curl -s -o /dev/null -w %{http_code} http://localhost:8083/connectors)
